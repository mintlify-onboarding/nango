...
| field (boolean) | parent\_nested (string) | list\_0 (number) | list\_1 (number) |
| --------------- | ----------------------- | ---------------- | ---------------- |
| true            | string\_value           | 1                | 2                |
...
Once a schema with data types has been generated, Nango will only store values in the SQL table that align with the data type of the schema. As an example, let's assume the field `num_users` has type number and these objects get returned by the API:
...
Nango will also change the schema of the generated table if the schema of the API response changes. Currently the following transformations are supported: 
...
Auto mapping is on by default for new Syncs. You can disable Auto Mapping for an individual Sync by setting the `auto_mapping` field to `false` in the [Sync config options](/nango-sync/use-nango/sync-all-options).

## Configuring a Sync's destination table[](#destination-table "Direct link to Configuring a Sync's destination table")

You can configure a Sync's destination database table with the `mapped_table` parameter in the [Sync config options](/nango-sync/use-nango/sync-all-options). 
...
We plan to introduce custom mappings soon. These will allow you to specify exactly (in code) how you want the JSON mapped to a SQL-table. This will enable a few interesting features:
