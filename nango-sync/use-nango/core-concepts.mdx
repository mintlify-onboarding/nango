...
* You [create Syncs](/nango-sync/use-nango/manage-syncs) (and manage them) from your application's code using the Nango SDK or REST API
...
Learn more about [creating & managing Syncs](/nango-sync/use-nango/sync-all-options) or look at the [full example of a Sync](#exampleSync) below.
...
* Nango automatically transforms the synced data from [JSON to SQL](/nango-sync/use-nango/schema-mappings)
* You can [attach Metadata](/nango-sync/use-nango/sync-metadata) to every record Nango syncs in (e.g. user id, account id)
