...
To get stared with OAuth, follow the instructions in the [Nango docs](/quickstart).

Once your have successfully created a Provider Configuration and Connection, specify the `nango_provider_config_key` and `nango_connection_id` parameters in your [Sync config options](/nango-sync/use-nango/sync-all-options) (cf. example below).
...
For other authentication methods that do not require frequent refreshes of credentials, you can simply specify the authentication information in the `headers`, `body` and `query_params` parameters in the [Sync config options](/nango-sync/use-nango/sync-all-options).
